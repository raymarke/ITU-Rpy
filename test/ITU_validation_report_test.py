...
class ITUR840_8TestCase(ITU_TestCase):
    ...
    def test_columnar_content_reduced_liquid(self):
        ...
        #         (45.00, -90, 65, 0.00000000),
        #     ]
        # df = pd.DataFrame(test_cases, columns=['lat', 'lon', 'p', 'Lred'])
    ...
    def test_cloud_attenuation(self):
        ...
        #                 (45.00, -90.00, 65.000, 45, 90.0, 9.05304465, 16.16757190, 0.68365520, 1.44305989, 0.00000000, 0.00000000),
        #             ]
        # selected_columns = [[row[0], row[1], row[3], row[4],row[2], row[-1]] for row in test_cases]

        # df = pd.DataFrame(selected_columns, columns=["lat", "lon", "f", "el", "p", "Ac"])
        # Run test and generate the report
        self.__run__(
...
class ITUR1623_1TestCase(ITU_TestCase):
    ...
    def test_fade_duration_number_fades(self):
        ...
        # Read the test data
        df = self.read_csv(
            path.join(test_data, path_file), columns=["D", "A", "el", "f", "T_tot", "N"]
        )

        # Run test and generate the report
        self.__run__(
            "test_fade_duration_number_fades",
